  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border/50 overflow-hidden hover-elevate transition-all duration-300">
      ...
      <div className="p-5 flex items-start justify-between">
        ...
        {post.coffeeShop && (
          <Link href={`/shops/${post.coffeeShop.id}`} className="flex items-center gap-1.5 text-xs font-medium text-primary bg-primary/5 px-3 py-1.5 rounded-full hover:bg-primary/10 transition-colors">
            <MapPin className="w-3.5 h-3.5" />
            <span className="truncate max-w-[120px]">{post.coffeeShop.name}</span>
          </Link>
        )}
      </div>
