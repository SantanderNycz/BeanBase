...
export default function CoffeeShopDetails() {
  ...
  const { data: posts, isLoading: postsLoading } = usePosts({ coffeeShopId: id });
  ...
  if (shopLoading) {
    return <div className="h-screen flex items-center justify-center text-primary animate-pulse">Loading...</div>;
  }
  ...
  const defaultBg = "https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=1920&h=600&fit=crop";
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="relative h-[40vh] sm:h-[50vh] w-full bg-secondary">
        ...
        <div className="absolute top-6 left-6">
          <Link href="/shops">
            <Button variant="outline" size="sm" className="bg-background/50 backdrop-blur-md border-transparent hover:bg-background/80 rounded-full">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative -mt-32">
        ...
        <div className="bg-card rounded-3xl p-6 sm:p-10 shadow-xl shadow-black/5 border border-border/50 mb-10">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
            <div className="space-y-4 flex-1">
              <h1 className="font-serif text-4xl sm:text-5xl font-bold text-foreground">{shop.name}</h1>
              <div className="flex items-center gap-2 text-muted-foreground bg-secondary/30 inline-flex px-4 py-2 rounded-full">
                <MapPin className="w-5 h-5 shrink-0 text-primary" />
                <span className="font-medium text-sm sm:text-base">{shop.address}</span>
              </div>
            ...
            <div className="flex sm:flex-col gap-3 shrink-0">
              ...
              >
                <Heart className={`w-5 h-5 mr-2 ${shop.isFavorite ? 'fill-current' : ''}`} />
                {shop.isFavorite ? 'Saved' : 'Favorite'}
              </Button>
              <Button variant="outline" size="lg" className="w-full sm:w-[160px] rounded-xl h-14" asChild>
                <a href={`https://maps.google.com/?q=${encodeURIComponent(shop.address)}`} target="_blank" rel="noreferrer">
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between border-b border-border/50 pb-4">
            <h2 className="font-serif text-2xl font-bold">Community Photos & Reviews</h2>
          ...
          {postsLoading ? (
            ...
            <div className="text-center py-16 bg-card rounded-3xl border border-border/50">
              <p className="text-muted-foreground text-lg mb-4">No reviews yet for {shop.name}.</p>
              <Link href="/">
